...
# Set multiprocessing start method to 'spawn' for Kaggle compatibility
try:
...
def save_logs(stats, label, tuning_hyperparameters):
    all_log_path = f"swintransformer/logs/all_experiments_{tuning_hyperparameters}.csv"
    os.makedirs("swintransformer/logs", exist_ok=True)

    log_path = os.path.join("swintransformer/logs", f"{label}.csv")
...
def plot_all_results(tuning_hyperparameters):
    df = pd.read_csv(f"swintransformer/logs/all_experiments_{tuning_hyperparameters}.csv")
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"swintransformer/logs/all_loss_plot_{tuning_hyperparameters}.png")
    plt.close()
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"swintransformer/logs/all_acc_plot_{tuning_hyperparameters}.png")
    plt.close()
...
def run_experiment(tuning_hyperparameters, config):
    ...
    model_instance = model.build_swintransformer(num_classes=100).to(device)
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        plot_epoch_results(stats, epoch, config['label'], f"swintransformer/logs/{config['label']}")
        ...
        os.makedirs(f"swintransformer/logs/{config['label']}", exist_ok=True)
        with open(f"swintransformer/logs/{config['label']}/epoch_{epoch}_results.json", 'w') as f:
            json.dump(epoch_results, f, indent=4)

        os.makedirs("swintransformer/models", exist_ok=True)
        torch.save(model_instance.state_dict(), f"swintransformer/models/{config['label']}_epoch_{epoch}.pth")
    ...
    with open(f"swintransformer/logs/train_times_{tuning_hyperparameters}.csv", "a", newline="") as f:
        writer = csv.writer(f)
        if os.stat(f"swintransformer/logs/train_times_{tuning_hyperparameters}.csv").st_size == 0:
...
if __name__ == "__main__":
    ...
    configs = [
        {'label': 'lr_001', 'lr': 0.001, 'batch_size': 4, 'optimizer': 'adam', 'weight_decay': 1e-4, 'epochs': 10},
        {'label': 'lr_0005', 'lr': 0.0005, 'batch_size': 4, 'optimizer': 'adam', 'weight_decay': 1e-4, 'epochs': 10},
        {'label': 'lr_0001', 'lr': 0.0001, 'batch_size': 4, 'optimizer': 'adam', 'weight_decay': 1e-4, 'epochs': 10},
    ]
