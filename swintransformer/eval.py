...
def evaluate_all_models(tuning_hyperparameters):
    ...
    model_dir = "swintransformer/models"
    results_dir = "swintransformer/results"
    os.makedirs(results_dir, exist_ok=True)

    loaders = dataset_loader.load_cifar100_datasets("data/", batch_size=4, num_workers=0, augment=False, val_split=0)
    test_loader = loaders['test']

    comparison_results = []
    train_times = pd.read_csv(f"swintransformer/logs/train_times_{tuning_hyperparameters}.csv").set_index('label')
