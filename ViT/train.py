...
from model_vit import build_vit_b16  # Thay VGG16 bằng ViT
...
def save_logs(stats, label, tuning_hyperparameters):
    all_log_path = f"vit/logs/all_experiments_{tuning_hyperparameters}.csv"
    os.makedirs("vit/logs", exist_ok=True)

    log_path = os.path.join("vit/logs", f"{label}.csv")
...
def run_experiment(tuning_hyperparameters, config):
    ...
    model = build_vit_b16(num_classes=100, pretrained=config['pretrained']).to(device)
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        print(f"[{config['label']}] Epoch {epoch}: Train {tr_loss:.4f}/{tr_acc:.2f}% | Val {val_loss:.4f}/{val_acc:.2f}%")
        ...
        if val_loss < best_val_loss:
            ...
            trigger_times = 0
            os.makedirs("vit/models", exist_ok=True)
            torch.save(model.state_dict(), f"vit/models/{config['label']}.pth")
        else:
            trigger_times += 1
    ...
    os.makedirs("vit/logs", exist_ok=True)
    with open(f"vit/logs/train_times_{tuning_hyperparameters}.csv", "a", newline="") as f:
        writer = csv.writer(f)
        if os.stat(f"vit/logs/train_times_{tuning_hyperparameters}.csv").st_size == 0:
...
if __name__ == "__main__":
    ...
    # configs = [
    #     {'label': 'vit_pretrained', 'lr': 0.01, 'batch_size': 128, 'optimizer': 'sgd', 'weight_decay': 1e-4, 'epochs': 50, 'aug_type': 'aug_none','scheduler': 'sched_cosine', 'pretrained': True},
    #     {'label': 'vit_not_pretrained', 'lr': 0.01, 'batch_size': 128, 'optimizer': 'sgd', 'weight_decay': 1e-4, 'epochs': 50, 'aug_type': 'aug_none','scheduler': 'sched_cosine', 'pretrained': False},
    # ]

    for config in configs:
        run_experiment(tuning_hyperparameters, config)
    print("All ViT experiments completed.")
