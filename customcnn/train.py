...
def save_logs(stats, label, tuning_hyperparameters):
    all_log_path = f"custom_cnn/logs/all_experiments_{tuning_hyperparameters}.csv"
    os.makedirs("custom_cnn/logs", exist_ok=True)

    log_path = os.path.join("custom_cnn/logs", f"{label}.csv")
...
def plot_all_results(tuning_hyperparameters):
    df = pd.read_csv(f"custom_cnn/logs/all_experiments_{tuning_hyperparameters}.csv")
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"custom_cnn/logs/all_loss_plot_{tuning_hyperparameters}.png")
    plt.close()
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"custom_cnn/logs/all_acc_plot_{tuning_hyperparameters}.png")
    plt.close()

def run_experiment(tuning_hyperparameters, config):
    print(f"Running config: {config['label']}")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    loaders = dataset_loader.load_cifar100_datasets("data/", batch_size=config['batch_size'], num_workers=2, augment=True, val_split=5000)
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        plot_epoch_results(stats, epoch, config['label'], f"custom_cnn/logs/{config['label']}")
        ...
        os.makedirs(f"custom_cnn/logs/{config['label']}", exist_ok=True)
        with open(f"custom_cnn/logs/{config['label']}/epoch_{epoch}_results.json", 'w') as f:
            json.dump(epoch_results, f, indent=4)

        os.makedirs("custom_cnn/models", exist_ok=True)
        torch.save(model_instance.state_dict(), f"custom_cnn/models/{config['label']}_epoch_{epoch}.pth")
    ...
    with open(f"custom_cnn/logs/train_times_{tuning_hyperparameters}.csv", "a", newline="") as f:
        writer = csv.writer(f)
        if os.stat(f"custom_cnn/logs/train_times_{tuning_hyperparameters}.csv").st_size == 0:
...
if __name__ == "__main__":
    ...
    configs = [
        {'label': 'lr_01', 'lr': 0.1, 'batch_size': 4, 'optimizer': 'adam', 'weight_decay': 1e-4, 'epochs': 30},
        {'label': 'lr_05', 'lr': 0.05, 'batch_size': 4, 'optimizer': 'adam', 'weight_decay': 1e-4, 'epochs': 30},
        {'label': 'lr_001', 'lr': 0.01, 'batch_size': 4, 'optimizer': 'adam', 'weight_decay': 1e-4, 'epochs': 30},
    ]
